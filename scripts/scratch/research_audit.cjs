...
async function simulatePersona(name, query, sector) {
    ...
    try {
        ...
        try {
            const segments = await YoutubeTranscript.fetchTranscript(video.id.videoId);
            transcript = segments.map(s => s.text).join(' ').substring(0, 500) + "...";
        } catch (e) {
            transcript = "Transcript unavailable for this specific video.";
        }
